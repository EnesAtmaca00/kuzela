    ...
    root.innerHTML = `
      <style>
        ...
        *{box-sizing:border-box}html{scroll-behavior:smooth}a{color:inherit;text-decoration:none}img{display:block;width:100%}h1,h2,h3,p{margin-top:0}.site{overflow:hidden;background:var(--paper)}.wrap{width:min(100% - 48px,1440px);margin-inline:auto}
        ...
        @media(max-width:900px){.wrap{width:min(100% - 36px,1440px)}.topbar{grid-template-columns:1fr auto;min-height:72px;padding-inline:18px}.nav{display:none}.brand{font-size:20px}.top-action{min-height:42px;padding-inline:17px;font-size:12px}.hero{min-height:790px;margin-top:-72px;padding:120px 18px 34px;background-image:linear-gradient(180deg,rgba(17,18,15,.24),rgba(17,18,15,.96) 72%),url('https://static.takeaway.com/images/restaurants/be/P01O0R13/products/mss_stel-je-eigen-pokebowl-medium_ebz.png');background-position:center;background-size:cover;background-repeat:no-repeat}h1{font-size:clamp(66px,20vw,104px)}.hero-bottom{display:block}.hero-copy{max-width:92%;margin-bottom:26px;font-size:17px}.intro{grid-template-columns:1fr;gap:38px;padding-block:100px}.proofs{grid-template-columns:1fr}.proof{min-height:auto;border-right:0;border-bottom:1px solid var(--line)}.proof:last-child{border-bottom:0}.proof span{margin-bottom:38px}.menu{padding-block:100px}.menu-head{display:block;margin-bottom:46px}.menu-head .pill{margin-top:28px}.grid{grid-template-columns:1fr}.story{grid-template-columns:1fr}.story-media{min-height:520px}.story-copy{min-height:680px;padding:70px 22px}.location-grid{grid-template-columns:1fr;gap:50px}.location-card{padding:28px}footer{flex-direction:column}}
