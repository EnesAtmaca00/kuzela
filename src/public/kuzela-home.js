    ...
    root.innerHTML = `
      <style>
        ...
        *{box-sizing:border-box}html{scroll-behavior:smooth}a{color:inherit;text-decoration:none}img{display:block;width:100%}h1,h2,h3,p{margin-top:0}.site{overflow:hidden;background:var(--paper)}.wrap{width:min(100% - 48px,1440px);margin-inline:auto}
        ...
        .brand{font-size:clamp(21px,2vw,30px);font-weight:900;letter-spacing:-.07em;text-transform:uppercase}.nav{display:flex;gap:28px;font-size:13px;font-weight:700}.nav a{opacity:.76;transition:opacity .25s}.nav a:hover{opacity:1}.top-action{justify-self:end}
        ...
        .hero{position:relative;min-height:910px;display:grid;align-items:end;margin-top:-84px;padding:150px clamp(24px,5vw,80px) 70px;color:var(--white);background:linear-gradient(90deg,rgba(9,10,8,.88),rgba(9,10,8,.62) 42%,rgba(9,10,8,.1) 76%),linear-gradient(180deg,transparent 55%,rgba(9,10,8,.72)),url('https://static.wixstatic.com/media/7279ae_9ec162d54ad64766b6bfa0def41bc811~mv2.jpg') 64% center/cover no-repeat}.hero-content{position:relative;z-index:1;max-width:1120px}.eyebrow{margin:0 0 22px;font-size:12px;font-weight:850;letter-spacing:.22em;text-transform:uppercase}
        h1{max-width:1100px;margin-bottom:30px;font-size:clamp(72px,10.4vw,166px);font-weight:900;letter-spacing:-.082em;line-height:.78;text-wrap:balance}.hero-bottom{display:flex;align-items:end;justify-content:space-between;gap:40px}.hero-copy{max-width:560px;margin:0;color:rgba(255,255,255,.84);font-size:clamp(17px,1.6vw,22px);line-height:1.48}.hero-buttons{display:flex;gap:12px;flex-wrap:wrap}
        ...
        @media(max-width:900px){.wrap{width:min(100% - 36px,1440px)}.topbar{grid-template-columns:1fr auto;min-height:72px;padding-inline:18px}.nav{display:none}.brand{font-size:20px}.top-action{min-height:42px;padding-inline:17px;font-size:12px}.hero{min-height:790px;margin-top:-72px;padding:120px 18px 34px;background-position:68% center}h1{font-size:clamp(66px,20vw,104px)}.hero-bottom{display:block}.hero-copy{max-width:92%;margin-bottom:26px;font-size:17px}.intro{grid-template-columns:1fr;gap:38px;padding-block:100px}.proofs{grid-template-columns:1fr}.proof{min-height:auto;border-right:0;border-bottom:1px solid var(--line)}.proof:last-child{border-bottom:0}.proof span{margin-bottom:38px}.menu{padding-block:100px}.menu-head{display:block;margin-bottom:46px}.menu-head .pill{margin-top:28px}.grid{grid-template-columns:1fr}.story{grid-template-columns:1fr}.story-media{min-height:520px}.story-copy{min-height:680px;padding:70px 22px}.location-grid{grid-template-columns:1fr;gap:50px}.location-card{padding:28px}footer{flex-direction:column}}
